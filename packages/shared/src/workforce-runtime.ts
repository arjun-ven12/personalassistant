...
export const WorkforceMatchScoreSchema = z
  .object({
    agentId: boundedRef,
...
export const SpecialistRequirementSchema = z
  .object({
...
export const WorkforceSpecialistProposalSchema = z
  .object({
...
export const WorkforceGapResolutionSchema = z
  .object({
...
export const WorkforceRuntimeTaskSchema = z
  .object({
    id: z.string().uuid(),
  ...
  agentDefinitionId: boundedRef.nullable().default(null),
  companyAssignmentId: z.string().uuid().nullable().default(null),
    ...
    sandboxStatus: z
      .enum(["NOT_REQUESTED", "PASSED", "FAILED", "UNAVAILABLE", "TIMED_OUT"])
      .nullable(),
...
export const WorkforceRuntimeMessageSchema = z
  .object({
    id: z.string().uuid(),
    ownerId: z.string().uuid(),
    organizationId: boundedRef.nullable(),
...
export const WorkforceRuntimeReviewSchema = z
  .object({
    id: z.string().uuid(),
    ownerId: z.string().uuid(),
    taskId: z.string().uuid(),
...
export const CreateWorkforceTaskRequestSchema = z
  .object({
    ...
    type: z.enum(["WORK", "QUESTION", "REVIEW"]).default("WORK"),
    title: z.string().trim().min(1).max(255),
    objective: z.string().trim().min(1).max(2_000),
    inputs: z.record(z.string().max(80), z.json()).default({}),
    evidenceRefs: z.array(boundedRef).max(50).default([]),
    memoryScopeRefs: z.array(boundedRef).max(20).default([]),
    requiredSkills: z.array(boundedRef).max(30).default([]),
    requiredCapabilities: z.array(boundedRef).max(30).default([]),
    preferredDepartmentId: boundedRef.nullable().default(null),
    priority: z.enum(["low", "normal", "high", "urgent"]).default("normal"),
    riskLevel: z.enum(["LOW", "MEDIUM", "HIGH"]).default("LOW"),
    economicBudget: z.number().int().min(1).max(1_000_000).default(10),
    maxRetries: z.number().int().min(0).max(2).default(1),
    expiresAt: z.iso.datetime().nullable().default(null),
  })
  .strict();

export const CreateWorkforceMessageRequestSchema = z
  .object({
    fromAgentId: boundedRef,
...
export const CompleteWorkforceTaskRequestSchema = z
  .object({
    resultSummary: z.string().trim().min(1).max(4_000),
    resultConfidence: z.number().min(0).max(1),
    actualCost: z.number().int().nonnegative().max(1_000_000),
    evidenceRefs: z.array(boundedRef).max(50).default([]),
...
export const SubmitWorkforceReviewRequestSchema = z
  .object({
    reviewerAgentId: boundedRef,
    verdict: WorkforceReviewVerdictSchema,
    findings: z.array(z.string().min(1).max(1_000)).max(50).default([]),
    evidenceRefs: z.array(boundedRef).max(50).default([]),
  })
  .strict();

export const WorkforceRuntimeDashboardSchema = z
  .object({
    summary: z
      .object({
    ...
    metrics: z
      .object({
        ...
        matchingLatencyMs: z.number().nonnegative(),
        peakActiveAgents: z.number().int().nonnegative(),
    ...
    invariants: z
      .object({
        ...
        dedicatedModelPerAgent: z.literal(false),
        hierarchyGrantsAuthority: z.literal(false),
        creditsGrantAuthority: z.literal(false),
        maxTaskDepth: z.literal(4),
      })
      .strict(),
  })
  .strict();

export const WorkforceRuntimeTaskResponseSchema = z
  .object({ task: WorkforceRuntimeTaskSchema })
  .strict();
export const WorkforceRuntimeMessageResponseSchema = z
  .object({ message: WorkforceRuntimeMessageSchema })
  .strict();
export const WorkforceRuntimeReviewResponseSchema = z
  .object({ task: WorkforceRuntimeTaskSchema, review: WorkforceRuntimeReviewSchema })
  .strict();
...
export type WorkforceSpecialistProposal = z.infer<
  typeof WorkforceSpecialistProposalSchema
>;
export type CreateWorkforceTaskRequest = z.input<
  typeof CreateWorkforceTaskRequestSchema
>;
export type CreateWorkforceMessageRequest = z.input<
  typeof CreateWorkforceMessageRequestSchema
>;
export type CompleteWorkforceTaskRequest = z.input<
  typeof CompleteWorkforceTaskRequestSchema
>;
export type SubmitWorkforceReviewRequest = z.input<
  typeof SubmitWorkforceReviewRequestSchema
>;
