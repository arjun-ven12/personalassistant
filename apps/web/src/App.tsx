...
const navigation: Array<{
  ...
  { path: "/conversation", label: "Conversation", icon: UserCircle, section: "Personal" },
...
export const App = ({ apiClient }: { apiClient: ApiClient }) => {
  ...
  const applicationTabs: WorkspaceTab[] = [
    { id: "applications", label: "Applications", content: <ApplicationsPage apiClient={apiClient} /> },
    {
      id: "capabilities",
      label: "Capabilities",
                      ...
                      {navigation
                        ...
                        .map(({ path, label, icon: Icon }) => (
                          ...
                          >
                            <Icon size={17} />
                            <span>{label}</span>
                            {path === "/approvals" && (pendingApprovals.data?.length ?? 0) > 0 ? (
                              <small className="nav-count">{pendingApprovals.data?.length}</small>
                            ) : null}
                          </Spatial>
                        ))}
                    </div>
