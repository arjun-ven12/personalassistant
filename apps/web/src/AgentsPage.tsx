...
export const AgentsPage = ({ apiClient }: { apiClient: ApiClient }) => {
  ...
  const [directoryMode, setDirectoryMode] = useState<"workforce" | "catalog">(
    "workforce",
  );
  ...
  const assignCatalogAgent = useMutation({
    mutationFn: apiClient.assignAgentFromCatalog,
    onSuccess: refresh,
  });
  const removeCatalogAgent = useMutation({
    mutationFn: apiClient.removeAgentAssignment,
    onSuccess: refresh,
  });
  ...
  const workspaceTabs = (
    <nav
      className="workspace-tabs agent-operations-tabs"
  ...
  if (workspaceView === "economy") {
    return (
      <section className="placeholder-page wide-page governance-page">
        ...
        <h1>Agent Economy</h1>
        <p>
          Internal credits buy bounded resources, never permissions, approvals,
          capabilities, trust, or reputation.
        </p>
        {workspaceTabs}
  ...
  if (workspaceView === "experiments") {
    return (
      <section className="placeholder-page wide-page governance-page">
        ...
        <p>
          Expose bounded objective experiments, variants, verified evidence, costs, and
          decisions without creating a second experiment engine.
        </p>
        {workspaceTabs}
  ...
  if (workspaceView === "activity") {
    return (
      <section className="placeholder-page wide-page governance-page">
        ...
        <p>
          CEO-readable operational events from the Business OS read model. Raw logs,
          prompts, and secrets stay out of this surface.
        </p>
        {workspaceTabs}
  ...
  if (workspaceView === "system") {
    return (
      <section className="placeholder-page wide-page governance-page">
        ...
        <h1>Agent System</h1>
        <p>
          Safe Agent OS, scheduler, AIRouter, data, and economy summaries from existing
          health and runtime APIs.
        </p>
        {workspaceTabs}
  ...
  if (workspaceView === "settings") {
    return (
      <section className="placeholder-page wide-page governance-page">
        ...
        <p>
          Safe owner-facing runtime configuration that already exists in backend
          contracts. No fabricated toggles or authority shortcuts.
        </p>
        {workspaceTabs}
    ...
    <section className="placeholder-page wide-page governance-page agents-control-page">
      ...
      <section className="status-grid">
        <article className="status-card">
          ...
          <small>Reusable specialists</small>
        </article>
        <article className="status-card">
          <span>Active</span>
          <strong>
        ...
        <article className="status-card">
          <span>Assigned</span>
          <strong>
            {catalog.data?.assignedCount ?? workforce.data?.summary.registered ?? 0}
          </strong>
          <small>Current company workforce</small>
        </article>
        <article className="status-card">
          <span>Open tasks</span>
          <strong>
            {(runtime.data?.summary.queued ?? 0) +
              (runtime.data?.summary.running ?? 0) +
              (runtime.data?.summary.waitingReview ?? 0)}
          </strong>
      ...
      <aside className="agents-side-rail" aria-label="Agent runtime controls">
        <section className="agent-rail-card agent-rail-runtime">
          ...
          <p>
            Sessions prepare bounded context only. They do not deploy work or bypass
            approvals.
          </p>
          <button
            disabled={!selectedAgent || startAgentOsSession.isPending}
            onClick={() =>
              startAgentOsSession.mutate({ agentId, inputSummary: agentOsInput })
            }
            type="button"
          >
          ...
          <input
            aria-label="Agent OS session context"
        ...
        <section className="agent-rail-card agent-rail-capabilities">
          ...
          {(dashboard.data?.dynamicWorkforce?.capabilities ?? [])
            .slice(0, 6)
            .map((capability) => (
              <div className="agent-capability-meter" key={capability.id}>
                <span>{capability.id}</span>
                <strong>{Math.round(capability.confidence * 100)}%</strong>
                <i>
                  <b style={{ width: `${Math.round(capability.confidence * 100)}%` }} />
                </i>
              </div>
            ))}
          {(dashboard.data?.dynamicWorkforce?.capabilities.length ?? 0) === 0 ? (
            <p className="agent-rail-empty">
              No bounded capability records are available.
            </p>
          ) : null}
        </section>
        ...
        <section className="agent-rail-card">
          ...
          <strong>
            {dashboard.data?.dynamicWorkforce?.dynamicAgents.length ?? 0} temporary
            agents active
          </strong>
        ...
        <section className="agent-rail-card agent-rail-adaptive">
          ...
          <textarea
            aria-label="Adaptive workflow goal"
            onChange={(event) => setTeamGoal(event.target.value)}
            rows={3}
            value={teamGoal}
          />
          <button
            disabled={composeTeam.isPending}
            onClick={() => composeTeam.mutate({ goal: teamGoal, repositoryIds: [] })}
            type="button"
          >
        ...
        <section className="agent-rail-card agent-rail-tasks">
          ...
          {(runtime.data?.tasks ?? [])
            ...
            .slice(0, 4)
            .map((task) => (
              <div className="agent-rail-task" key={task.id}>
                <strong>{task.title}</strong>
                <small>
                  {task.status} · {task.assignedAgentId ?? "matching"}
                </small>
              </div>
            ))}
          {(runtime.data?.tasks.length ?? 0) === 0 ? (
            <p className="agent-rail-empty">No live workforce tasks.</p>
          ) : null}
        </section>
      ...
      <div className="agents-main-column">
        <section className="panel-list workforce-runtime-panel">
          <div className="agent-runtime-heading">
            <div>
              <p className="eyebrow">On-demand workforce</p>
              <h2>Live task runtime</h2>
            </div>
            <span>
              {runtime.data?.summary.active ?? 0} active ·{" "}
              {runtime.data?.summary.dormant ?? 0} dormant ·{" "}
              {runtime.data?.metrics.providerCalls ?? 0} provider calls ·{" "}
              {Math.round(runtime.data?.metrics.matchingLatencyMs ?? 0)} ms match
            </span>
          </div>
          <form
            className="workforce-runtime-create"
            onSubmit={(event) => {
              event.preventDefault();
              queueRuntimeTask.mutate({
                ...
                economicBudget: 10,
                evidenceRefs: ["owner:dashboard"],
              });
            }}
          >
            <input
              aria-label="Runtime task title"
              onChange={(event) => setTitle(event.target.value)}
              value={title}
            />
            <input
              aria-label="Runtime task objective"
              onChange={(event) => setObjective(event.target.value)}
              value={objective}
            />
            <button disabled={queueRuntimeTask.isPending} type="submit">
              Queue task
            </button>
          </form>
          <div className="workforce-runtime-list">
            {(runtime.data?.tasks ?? []).slice(0, 8).map((task) => {
              ...
              return (
                <article key={task.id}>
                  <i
                    className={`runtime-task-state state-${task.status.toLowerCase()}`}
                  />
                  <div>
                    <strong>{task.title}</strong>
                    <span>
                      {task.assignedAgentId ?? "Awaiting deterministic match"} · depth{" "}
                      {task.depth} · {task.actualCost || task.reservedCredits} credits
                    </span>
                  </div>
                  {selected ? (
                    <small>
                      {selected.category.replaceAll("_", " ")} · match{" "}
                      {Math.round(selected.finalScore * 100)}% · skills{" "}
                      {Math.round(selected.skillFit * 100)}% · capabilities{" "}
                      {Math.round(selected.capabilityFit * 100)}%
                    </small>
                  ) : (
                    <small>{task.status}</small>
                  )}
                  {proposal ? (
                    <div className="runtime-task-gap">
                  ...
                  <div className="runtime-task-actions">
                    {task.status === "QUEUED" ? (
                      <button
                        disabled={runRuntimeTask.isPending}
                        onClick={() => runRuntimeTask.mutate(task.id)}
                        type="button"
                      >
                        Run
                      </button>
                    ...
                    {proposal &&
                    task.workforceGap?.decision === "SPECIALIST_APPROVAL_PENDING" ? (
                      <button
                    ...
                    {!["COMPLETED", "FAILED", "CANCELLED", "EXPIRED"].includes(
                      ...
                      <button
                        disabled={cancelRuntimeTask.isPending}
                        onClick={() => cancelRuntimeTask.mutate(task.id)}
                        type="button"
                      >
                        Cancel
                      </button>
            ...
            {(runtime.data?.tasks.length ?? 0) === 0 ? (
              <p className="agent-rail-empty">
                Queue a bounded task to activate one specialist through the shared
                runtime.
              </p>
            ) : null}
          </div>
        ...
        <section className="panel-list agent-society-panel">
          <h2>Agent Society</h2>
          <p>
            Agent Society organizes specialists into departments, task forces,
            leadership roles, debates, consensus sessions, peer review, mentorship,
            communications, reputation, and organizational memory. These are
            coordination records only: they do not grant permissions, approve work, or
            execute anything.
          </p>
          <section className="status-grid">
            <article className="status-card">
        ...
        <section className="panel-list agent-directory-panel">
          <div className="agent-directory-heading">
            <div>
              ...
              <h2>
                {directoryMode === "workforce" ? "Company workforce" : "Agent catalog"}
              </h2>
            </div>
            <small>
              {directoryMode === "workforce"
                ? `${directoryAgents.length} of ${catalog.data?.assignedCount ?? 0}`
                : `${catalog.data?.items.length ?? 0} of ${catalog.data?.catalogCount ?? 0}`}
            </small>
          </div>
          <div className="agent-directory-toolbar">
            ...
            />
            <div
              className="agent-directory-modes"
              role="group"
              aria-label="Agent directory view"
            >
              <button
                className={directoryMode === "workforce" ? "active" : ""}
                onClick={() => setDirectoryMode("workforce")}
                type="button"
              >
                Assigned
              </button>
              <button
                className={directoryMode === "catalog" ? "active" : ""}
                onClick={() => setDirectoryMode("catalog")}
                type="button"
              >
                Catalog
              </button>
          ...
          {directoryMode === "workforce" ? (
            <div
              className="agent-directory-table"
              role="table"
              aria-label="Company workforce directory"
            >
              <div className="agent-directory-row agent-directory-header" role="row">
                <span>Agent</span>
              ...
              {directoryAgents.map((agent) => {
                const currentTask = runtime.data?.tasks.find(
                  (task) =>
                ...
                return (
                  <button
                    ...
                    key={agent.id}
                    onClick={() => setWorkspaceView("workforce")}
                    role="row"
                    type="button"
                  >
                    ...
                    <span>
                      <i
                        className={`agent-health-dot state-${agent.status.toLowerCase()}`}
                      />
                      {currentTask
              ...
              {directoryAgents.length === 0 ? (
                <p className="notice">No assigned specialists match this search.</p>
              ) : null}
            </div>
          ) : (
            <div
              className="agent-catalog-table"
              role="table"
              aria-label="Reusable agent catalog"
            >
              <div className="agent-catalog-row agent-directory-header" role="row">
              ...
              {(catalog.data?.items ?? [])
                .filter((item) =>
                  `${item.definition.name} ${item.definition.description} ${item.definition.skills.join(" ")}`
                    .toLowerCase()
                    .includes(directoryQuery.trim().toLowerCase()),
                )
                .map((item) => (
                  ...
                  >
                    <span>
                      ...
                      <small>Reusable · v{item.definition.version}</small>
                    </span>
                    <span>{item.definition.skills.slice(0, 3).join(" · ")}</span>
                    <span>
                      <i
                        className={`agent-health-dot state-${item.currentCompanyStatus.toLowerCase()}`}
                      />
                      {item.currentCompanyStatus.toLowerCase().replaceAll("_", " ")}
                    </span>
                    <span>
                      {item.assignedCompanyCount}{" "}
                      {item.assignedCompanyCount === 1 ? "company" : "companies"}
                    </span>
                    <span>
                      {item.currentCompanyStatus === "ASSIGNED" ? (
                        <button
                          disabled={
                            item.assignment?.isGovernor || removeCatalogAgent.isPending
                          }
                          onClick={() => removeCatalogAgent.mutate(item.definition.id)}
                          type="button"
                        >
                          {item.assignment?.isGovernor ? "Required" : "Remove"}
                        </button>
                      ) : (
                        <button
                          disabled={
                            item.currentCompanyStatus !== "AVAILABLE" ||
                            assignCatalogAgent.isPending
                          }
                          onClick={() => assignCatalogAgent.mutate(item.definition.id)}
                          type="button"
                        >
                          Assign
                        </button>
...
const AgentExperimentsPage = ({ apiClient }: { apiClient: ApiClient }) => {
  ...
  const selected =
    experiments.data?.experiments.find((item) => item.id === selectedId) ??
    experiments.data?.experiments[0];
  const variants = (experiments.data?.variants ?? []).filter(
    (item) => item.experimentId === selected?.id,
  );
  const results = (experiments.data?.results ?? []).filter(
    (item) => item.experimentId === selected?.id,
  );
  const timeline = (experiments.data?.timeline ?? [])
    .filter((item) => item.experimentId === selected?.id)
    .slice(0, 12);
  const running =
    experiments.data?.experiments.filter((item) => item.status === "RUNNING") ?? [];
  const needsAttention =
    experiments.data?.experiments.filter((item) =>
      ["PAUSED", "FAILED", "STOPPED"].includes(item.status),
    ) ?? [];
  const completed =
    experiments.data?.experiments.filter((item) => item.status === "COMPLETED") ?? [];

  if (experiments.isLoading)
    return <div className="notice">Loading experiments...</div>;
  ...
  return (
    <div className="agent-operations-page">
      <section className="compact-metric-strip" aria-label="Experiment summary">
        <span>
          <small>Running</small>
          <strong>{experiments.data?.summary.running ?? 0}</strong>
        </span>
        <span>
        ...
        <span>
          ...
          <strong>{experiments.data?.summary.completed ?? 0}</strong>
        </span>
        <span>
          <small>Budget allocated</small>
          <strong>{experiments.data?.summary.budgetAllocated ?? 0}</strong>
        </span>
        <span>
          <small>Budget spent</small>
          <strong>{experiments.data?.summary.budgetSpent ?? 0}</strong>
        </span>
      </section>
      ...
      <div className="split-workspace">
        <section className="panel-list">
          ...
          {experiments.data?.experiments.length === 0 ? (
            <div className="notice">No experiments are currently running.</div>
          ) : null}
        </section>
          ...
          {selected ? (
            <>
              <dl className="compact-definition-list">
                ...
                <div>
                  <dt>Objective</dt>
                  <dd>{selected.objectiveId}</dd>
                </div>
                <div>
                  <dt>Project</dt>
                  <dd>{selected.projectId ?? "None"}</dd>
                </div>
                <div>
                  <dt>Metric</dt>
                  <dd>{selected.primaryMetric.name}</dd>
                </div>
                <div>
                  <dt>Budget</dt>
                  <dd>
                    {selected.spentCredits} / {selected.explorationBudget}
                  </dd>
                </div>
                <div>
                  <dt>Decision</dt>
                  <dd>{decisionFor(results)}</dd>
                </div>
              </dl>
              <article className="panel">
                ...
                {variants.map((variant) => (
                  <div className="dense-row" key={variant.id}>
                    <div>
                      ...
                      <small>
                        {variant.role} · {variant.status} · allocation{" "}
                        {variant.allocationPercent}%
                      </small>
                    </div>
                    <div className="row-meta">
                      <span className="mono-number">
                        {variant.actualMetric ?? "no result"}
                      </span>
...
const ExperimentSection = ({
  ...
  <section className="agent-ops-section">
    <div className="section-heading-row">
      ...
      <span className="mono-meta">{experiments.length}</span>
    </div>
    <div className="dense-list">
      {experiments.map((experiment) => (
        <button
          className={`dense-row as-button${selectedId === experiment.id ? " is-selected" : ""}`}
          key={experiment.id}
        ...
        >
          ...
          <div className="row-meta">
            ...
            <span className="mono-number">
              {experiment.spentCredits}/{experiment.explorationBudget}
            </span>
          </div>
        </button>
      ))}
      {experiments.length === 0 ? (
        <div className="notice">No {title.toLowerCase()} experiments.</div>
      ) : null}
    </div>
  </section>
);
    ...
    <div className="agent-operations-page">
      <section className="compact-metric-strip" aria-label="Activity summary">
        <span>
          ...
          <strong>{businessOS.data?.summary.attentionCount ?? 0}</strong>
        </span>
        <span>
          <small>Critical</small>
          <strong>{businessOS.data?.summary.criticalAlerts ?? 0}</strong>
        </span>
        <span>
          <small>Approvals</small>
          <strong>{businessOS.data?.summary.pendingApprovals ?? 0}</strong>
        </span>
        <span>
          <small>Outcomes</small>
          <strong>{businessOS.data?.summary.verifiedOutcomes ?? 0}</strong>
        </span>
      </section>
      <section className="panel-list">
        <div className="section-heading-row">
          ...
          <span className="mono-meta">{events.length} shown</span>
        </div>
        <div className="dense-list">
          {events.map((event) => (
            <div className="dense-row" key={event.id}>
              <div>
                ...
                <small>
                  {event.category} · {event.entity?.label ?? "System"}
                </small>
              </div>
              <div className="row-meta">
                <span>{event.entity?.status ?? "recorded"}</span>
          ...
          {events.length === 0 ? (
            <div className="notice">No business activity recorded.</div>
          ) : null}
        </div>
        {(businessOS.data?.timeline.length ?? 0) > visible ? (
          <button
            className="secondary-button"
            onClick={() => setVisible((value) => value + 40)}
            type="button"
          >
            Load more activity
          </button>
...
const AgentSystemPage = ({ apiClient }: { apiClient: ApiClient }) => {
  const system = useQuery({
    queryKey: ["system-status", "agent-ops"],
    queryFn: apiClient.getSystemStatus,
    refetchInterval: 30_000,
  });
  const workforce = useQuery({
    queryKey: ["workforce-runtime", "system"],
    queryFn: apiClient.getWorkforceRuntime,
    refetchInterval: 15_000,
  });
  const graph = useQuery({
    queryKey: ["agent-workforce-graph", "system"],
    queryFn: () => apiClient.getAgentWorkforceGraph("limit=160"),
    refetchInterval: 30_000,
  });
  const economy = useQuery({
    queryKey: ["agent-economy-dashboard", "system"],
    queryFn: apiClient.getAgentEconomyDashboard,
    refetchInterval: 30_000,
  });
  const agentOS = useQuery({
    queryKey: ["agent-os-dashboard", "system"],
    queryFn: apiClient.getAgentOsDashboard,
    refetchInterval: 30_000,
  });
  const providers = useQuery({
    queryKey: ["ai-providers", "system"],
    queryFn: apiClient.getAIProviderHealth,
    refetchInterval: 30_000,
  });
  const providerAvailable =
    providers.data?.filter((item) => item.status === "HEALTHY").length ?? 0;
    ...
    <div className="agent-operations-page">
      <section className="status-grid">
        <article className="status-card">
          ...
          <strong>{graph.data?.summary.registered ?? 0}</strong>
          <small>Agent OS identities</small>
        </article>
        <article className="status-card">
          <span>Active</span>
          <strong>{graph.data?.summary.active ?? 0}</strong>
          <small>Runtime participants</small>
        </article>
        <article className="status-card">
          <span>Dormant</span>
          <strong>{graph.data?.summary.dormant ?? 0}</strong>
          <small>No model sessions</small>
        </article>
        <article className="status-card">
          <span>Open tasks</span>
          <strong>
            {(workforce.data?.summary.queued ?? 0) +
              (workforce.data?.summary.running ?? 0) +
              (workforce.data?.summary.waitingReview ?? 0)}
          </strong>
          <small>Scheduler backlog</small>
        </article>
      </section>
      <div className="split-workspace">
        <section className="panel-list">
          <div className="section-heading-row">
            <div>
              ...
              <h2>Scheduler and Agent OS</h2>
            </div>
          </div>
          <dl className="compact-definition-list">
            <div>
              <dt>Scheduler</dt>
              <dd>{workforce.data ? "HEALTHY" : "Unavailable"}</dd>
            </div>
            <div>
              <dt>Max concurrent</dt>
              <dd>{workforce.data?.summary.maxConcurrent ?? "-"}</dd>
            </div>
            <div>
              <dt>Active workflows</dt>
              <dd>{workforce.data?.summary.running ?? 0}</dd>
            </div>
            <div>
              <dt>Review queue</dt>
              <dd>{workforce.data?.summary.waitingReview ?? 0}</dd>
            </div>
            <div>
              <dt>Manifests</dt>
              <dd>{agentOS.data?.manifests.length ?? 0}</dd>
            </div>
            <div>
              <dt>Sessions</dt>
              <dd>{agentOS.data?.sessions.length ?? 0}</dd>
            </div>
          </dl>
        </section>
        <section className="panel-list">
          <div className="section-heading-row">
            <div>
              ...
              <h2>Safe component health</h2>
            </div>
          </div>
          <dl className="compact-definition-list">
            <div>
              <dt>API</dt>
              <dd>{system.data?.api.status ?? "unknown"}</dd>
            </div>
            <div>
              <dt>PostgreSQL</dt>
              <dd>{system.data?.database.status ?? "unknown"}</dd>
            </div>
            <div>
              <dt>Redis</dt>
              <dd>{system.data?.redis.status ?? "unknown"}</dd>
            </div>
            <div>
              <dt>AIRouter</dt>
              <dd>{system.data?.aiProvider.status ?? "unknown"}</dd>
            </div>
            <div>
              <dt>Providers available</dt>
              <dd>{providerAvailable}</dd>
            </div>
            <div>
              <dt>Execution enabled</dt>
              <dd>{system.data?.execution.enabled ? "Yes" : "No"}</dd>
            </div>
          </dl>
        </section>
      </div>
      <section className="compact-metric-strip" aria-label="System economy">
        <span>
          ...
          <strong>{economy.data?.overview.availableCredits ?? 0}</strong>
        </span>
        <span>
          <small>Reserved credits</small>
          <strong>{economy.data?.overview.reservedCredits ?? 0}</strong>
        </span>
        <span>
          <small>Spent credits</small>
          <strong>{economy.data?.overview.spentCredits ?? 0}</strong>
        </span>
        <span>
          <small>Completion rate</small>
          <strong>
            {Math.round((workforce.data?.metrics.completionRate ?? 0) * 100)}%
          </strong>
        </span>
      </section>
    </div>
...
const AgentSettingsPage = ({ apiClient }: { apiClient: ApiClient }) => {
  const workforce = useQuery({
    queryKey: ["workforce-runtime", "settings"],
    queryFn: apiClient.getWorkforceRuntime,
  });
  const economy = useQuery({
    queryKey: ["agent-economy-dashboard", "settings"],
    queryFn: apiClient.getAgentEconomyDashboard,
  });
  const experiments = useQuery({
    queryKey: ["experiments", "settings"],
    queryFn: apiClient.getExperiments,
  });
  const agentOS = useQuery({
    queryKey: ["agent-os-dashboard", "settings"],
    queryFn: apiClient.getAgentOsDashboard,
  });
    ...
    <div className="agent-operations-page">
      <section className="panel-list">
        <div className="section-heading-row">
          ...
          <span className="mono-meta">Read-only</span>
        </div>
        <dl className="compact-definition-list">
          <div>
            <dt>Max concurrent active agents</dt>
            <dd>{workforce.data?.summary.maxConcurrent ?? "-"}</dd>
          </div>
          <div>
            <dt>Max task depth</dt>
            <dd>{workforce.data?.invariants.maxTaskDepth ?? "-"}</dd>
          </div>
          <div>
            <dt>Shared AIRouter</dt>
            <dd>{workforce.data?.invariants.sharedAIRouter ? "Enabled" : "Unknown"}</dd>
          </div>
          <div>
            <dt>Dedicated model per agent</dt>
            <dd>{workforce.data?.invariants.dedicatedModelPerAgent ? "Yes" : "No"}</dd>
          </div>
        </dl>
      </section>
      <section className="panel-list">
        <div className="section-heading-row">
          ...
          <span className="mono-meta">Credits are not reputation</span>
        </div>
        <dl className="compact-definition-list">
          <div>
            <dt>Grant authority</dt>
            <dd>{economy.data?.creditsGrantAuthority ?? "-"}</dd>
          </div>
          <div>
            <dt>Credits buy authority</dt>
            <dd>{economy.data?.creditsCanBuyAuthority ? "Yes" : "No"}</dd>
          </div>
          <div>
            <dt>Credits buy reputation</dt>
            <dd>{economy.data?.creditsCanBuyReputation ? "Yes" : "No"}</dd>
          </div>
          <div>
            <dt>Economy accounts</dt>
            <dd>{economy.data?.accounts.length ?? 0}</dd>
          </div>
        </dl>
      </section>
      <section className="panel-list">
        <div className="section-heading-row">
          ...
          <span className="mono-meta">Existing engine</span>
        </div>
        <dl className="compact-definition-list">
          <div>
            <dt>Experiments grant authority</dt>
            <dd>
              {experiments.data?.invariants.experimentsGrantAuthority ? "Yes" : "No"}
            </dd>
          </div>
          <div>
            <dt>Verified evidence only</dt>
            <dd>{experiments.data?.invariants.verifiedEvidenceOnly ? "Yes" : "No"}</dd>
          </div>
          <div>
            <dt>Objective budget conserved</dt>
            <dd>
              {experiments.data?.invariants.objectiveBudgetConserved ? "Yes" : "No"}
            </dd>
          </div>
          <div>
            <dt>Existing scheduler used</dt>
            <dd>{experiments.data?.invariants.existingSchedulerUsed ? "Yes" : "No"}</dd>
          </div>
        </dl>
      </section>
      <section className="panel-list">
        <div className="section-heading-row">
          <div>
            <p className="eyebrow">AI and memory</p>
        ...
        <div className="dense-list">
          {(agentOS.data?.configurations ?? []).slice(0, 20).map((configuration) => (
            <div className="dense-row" key={configuration.id}>
              <div>
                <strong>{configuration.agentId}</strong>
                <small>
                  parallelism {configuration.configuration.parallelism} · memory{" "}
                  {configuration.configuration.memoryLimitItems}
                </small>
              </div>
              <div className="row-meta">
                <span>{configuration.configuration.defaultModel}</span>
                <span>
                  {configuration.signedChangeRequired ? "signed changes" : "unsigned"}
                </span>
              </div>
            </div>
          ))}
          {agentOS.data?.configurations.length === 0 ? (
            <div className="notice">No Agent OS configurations registered.</div>
          ) : null}
        </div>
      </section>
    </div>
