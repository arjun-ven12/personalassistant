...
describe.skipIf(!connectionString)("Phase 21D PostgreSQL skill evolution persistence", () => {
  ...
  it("survives service reconstruction with owner isolation and active pointer intact", async () => {
    ...
    await first.suppressCandidate({
      ownerId: ownerA,
      body: { candidateId: candidateDashboard.candidates[0]!.id, reason: "Do not suggest again." },
      requestId: "pg-6",
      ipAddress: "127.0.0.1",
    });
    ...
    for (let i = 0; i < 6; i += 1) await first.recordUsage(ownerA, skill.id, i < 4 ? "FAILED" : "SUCCESS");
    ...
    expect(recovered.evaluations.some((item) => item.status === "ROLLBACK_RECOMMENDED")).toBe(true);
