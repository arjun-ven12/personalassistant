...
export const registerObjectiveRoutes=(app:FastifyInstance,context:ApiRouteContext)=>{
  app.get("/api/objectives",{preHandler:[context.security.requireAuthentication]},async(request)=>ObjectiveDashboardSchema.parse(await context.objectives.dashboard(context.security.getIdentity(request).user.id)));
  app.post("/api/objectives",{preHandler:[context.security.requireAuthentication,context.security.requireTrustedOrigin,context.security.requireCsrf]},async(request)=>ObjectiveDraftResponseSchema.parse(await context.objectives.create({ownerId:context.security.getIdentity(request).user.id,body:CreateObjectiveRequestSchema.parse(request.body),requestId:request.id,ipAddress:request.ip})));
  for(const action of ["activate","pause","cancel","replan"] as const) app.post(`/api/objectives/:objectiveId/${action}`,{preHandler:[context.security.requireAuthentication,context.security.requireTrustedOrigin,context.security.requireCsrf]},async(request)=>{
  ...
  app.patch("/api/objectives/:objectiveId",{preHandler:[context.security.requireAuthentication,context.security.requireTrustedOrigin,context.security.requireCsrf]},async(request)=>{
  ...
  app.post("/api/objectives/:objectiveId/observations",{preHandler:[context.security.requireAuthentication,context.security.requireTrustedOrigin,context.security.requireCsrf]},async(request)=>{
    const ownerId=context.security.getIdentity(request).user.id; const {objectiveId}=Params.parse(request.params);
