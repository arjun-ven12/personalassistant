...
describe("AgentWorkforceService", () => {
  it("bootstraps more than 100 meaningful dormant agents without runtime activation", async () => {
    ...
    expect(agents.every((agent) => agent.workforce?.activationPolicyId === "lazy_owner_or_task_activation_v1")).toBe(true);
    expect(agents.every((agent) => agent.configuration.externalToolDeclarationsImported === false)).toBe(true);
    expect(dashboard.accounts.every((account) => account.economyStatus === "DORMANT")).toBe(true);
  ...
  it("is idempotent and preserves external provenance without importing external authority", async () => {
    ...
    const external = second.filter((agent) => agent.workforce?.source === "EVERYTHING_CLAUDE_CODE");

    expect(second).toHaveLength(first.length);
    expect(new Set(second.map((agent) => agent.id)).size).toBe(second.length);
    expect(external.length).toBeGreaterThan(50);
    expect(external.every((agent) => agent.workforce?.sourceVersion === "d8409a4b0813771235555e32e3d8046a73988bfa")).toBe(true);
    expect(external.every((agent) => agent.workforce?.license === "MIT")).toBe(true);
    expect(external.every((agent) => agent.configuration.externalToolDeclarationsImported === false)).toBe(true);
  });
  ...
  it("assigns isolated private memory with shared department and organization scopes", async () => {
    ...
    const development = agents.filter((agent) => agent.workforce?.departmentMemoryScopeId.includes("department:"));
    const first = development[0]!;
    const peer = development.find((agent) => agent.id !== first.id && agent.workforce?.departmentId === first.workforce?.departmentId)!;
    const other = development.find((agent) => agent.workforce?.departmentId !== first.workforce?.departmentId)!;

    expect(first.workforce?.memoryScopeId).not.toBe(peer.workforce?.memoryScopeId);
    expect(first.workforce?.departmentMemoryScopeId).toBe(peer.workforce?.departmentMemoryScopeId);
    expect(first.workforce?.departmentMemoryScopeId).not.toBe(other.workforce?.departmentMemoryScopeId);
    expect(first.workforce?.organizationMemoryScopeId).toBe(other.workforce?.organizationMemoryScopeId);
  });
  ...
  it("uses finite role profiles instead of inherited manager authority", async () => {
    ...
    await workforce.bootstrap(ownerId, "request", "127.0.0.1");
    const agents = agentStore.listAgents(ownerId);
    const security = agents.find((agent) => agent.role === "security" && agent.workforce)!;
  ...
  it("rejects hierarchy cycles and changes participation without creating a model runtime", async () => {
    ...
    expect(() => workforce.validateHierarchy([
    ...
    const activated = await workforce.setActivation(ownerId, first.id, "ACTIVE", "activate", "127.0.0.1");
    expect(activated.economy?.economyStatus).toBe("ACTIVE");
    expect(activated.agent.configuration.runtimeMode).toBe("LAZY_SHARED_AI");
    expect(workforceStore.listEvents(ownerId, first.id, 20).some((event) => event.type === "ACTIVATED")).toBe(true);
    const dormant = await workforce.setActivation(ownerId, first.id, "DORMANT", "dormant", "127.0.0.1");
  ...
  it("supports bounded graph search and owner-isolated detail", async () => {
    ...
    await workforce.bootstrap(ownerId, "request", "127.0.0.1");
    const graph = await workforce.graph(ownerId, { q: "native_security_lead", limit: 500 });
    ...
    await expect(workforce.detail("22222222-2222-4222-8222-222222222222", agents[0]!.id)).rejects.toMatchObject({ code: "AGENT_NOT_FOUND" });
  });
});
