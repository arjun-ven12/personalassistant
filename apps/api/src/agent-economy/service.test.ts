...
const activateFunded = async (
  ...
  await service.allocate({
    ...
    agentId,
    amount,
    reasonCode: "TEST_ALLOCATION",
    idempotencyKey: `allocate:${agentId}:${amount}`,
    requestId: "request-allocate",
    ipAddress: "127.0.0.1",
  });
...
describe("AgentEconomyService", () => {
  it("enrolls 250 dormant agents without runtime or provider activation", async () => {
    ...
    expect(
      dashboard.accounts.every(
        (account) =>
          account.availableCredits === 0 && account.economyStatus === "DORMANT",
      ),
    ).toBe(true);
  });

  it("keeps dormant economy enrollment independent for the same reusable agent in two companies", async () => {
    const { agents, service } = setup();
    const atlasId = "33333333-3333-4333-8333-333333333333";
    const novaId = "44444444-4444-4444-8444-444444444444";
    await companyScope.run(
      { ownerId, companyId: atlasId, role: "OWNER", requestId: "atlas-enroll" },
      async () => {
        addAgent(agents, "shared_specialist");
        ...
        expect((await service.dashboard(ownerId)).overview.dormantAgents).toBe(1);
      },
    );
    await companyScope.run(
      { ownerId, companyId: novaId, role: "OWNER", requestId: "nova-enroll" },
      async () => {
        addAgent(agents, "shared_specialist");
        await service.enroll(
          ownerId,
  ...
  it("uses an idempotent ledger for owner allocations", async () => {
    ...
    const input = {
      ...
      agentId: "agent_a",
      amount: 40,
      reasonCode: "OWNER_ALLOCATION",
      idempotencyKey: "allocation:stable",
      requestId: "request",
      ipAddress: "127.0.0.1",
    };
  ...
  it("reserves, settles actual cost, and releases the remainder", async () => {
    ...
    const reserved = await service.reserve({
      ...
      reasonCode: "MODEL_ESTIMATE",
      idempotencyKey: "provider:req-1",
      references: { providerRequestId: "req-1" },
    });
    ...
    expect(await service.reconstructBalance(ownerId, "agent_a")).toEqual({
      availableCredits: 86,
      reservedCredits: 0,
      lifetimeEarned: 100,
      lifetimeSpent: 14,
  ...
  it("denies self reward and exposes no authority-purchase operation", async () => {
    ...
    await activateFunded(service);
    await expect(
      service.rewardVerified({
        ownerId,
        agentId: "agent_a",
        amount: 10,
        authority: "AGENT" as never,
        idempotencyKey: "reward:self",
        reasonCode: "SELF",
        outcome: {
          taskId: "task-1",
    ...
    ).rejects.toMatchObject({ code: "SELF_REWARD_DENIED" });
    ...
    ).rejects.toMatchObject({ code: "FAILED_OUTCOME_REWARD_DENIED" });
  });
  ...
  it("rewards a duplicate verified callback only once and updates calibration", async () => {
    ...
    const input = {
      ...
      agentId: "agent_a",
      amount: 20,
      authority: "TASK_VERIFIER" as const,
      idempotencyKey: "task:verified-1",
      reasonCode: "VERIFIED_TASK",
      outcome: {
        taskId: "task-1",
  ...
  it("keeps owners and agents isolated", async () => {
    ...
    await expect(
      service.allocate({
        ...
        agentId: "other_agent",
        amount: 10,
        reasonCode: "BAD_SCOPE",
        idempotencyKey: "allocation:other",
        requestId: "request",
        ipAddress: "127.0.0.1",
      }),
  ...
  it("blocks reservations when Agent OS lifecycle is paused", async () => {
    ...
    await expect(
      service.reserve({
        ownerId,
        agentId: "paused_agent",
        amount: 1,
        costType: "TASK_EXECUTION",
        reasonCode: "TASK",
        idempotencyKey: "reserve:paused",
      }),
    ).rejects.toMatchObject({ code: "AGENT_LIFECYCLE_BLOCKS_ECONOMY" });
  });
  ...
  it("scores calibrated predictions above overconfident failures", async () => {
    ...
    for (let index = 0; index < 10; index += 1) {
      const aOutcome = {
        ...
        estimatedCost: 2,
        estimatedDurationMs: 100,
        actualSuccess: index < 5,
        actualCost: 2,
        actualDurationMs: 100,
        qualityScore: 0.7,
        verificationResult: "VERIFIED" as const,
        evidenceRefs: [`a-review-${index}`],
      };
      const bOutcome = {
        taskId: `b-task-${index}`,
        predictedSuccessProbability: 0.7,
        estimatedCost: 2,
        estimatedDurationMs: 100,
        actualSuccess: index < 7,
        actualCost: 2,
        actualDurationMs: 100,
        qualityScore: 0.7,
        verificationResult: "VERIFIED" as const,
        evidenceRefs: [`b-review-${index}`],
      };
      if (aOutcome.actualSuccess)
        await service.rewardVerified({
          ownerId,
          agentId: "agent_a",
          amount: 1,
          authority: "TASK_VERIFIER",
      ...
      if (bOutcome.actualSuccess)
        await service.rewardVerified({
          ownerId,
          agentId: "agent_b",
          amount: 1,
          authority: "TASK_VERIFIER",
          idempotencyKey: `b:${index}`,
          reasonCode: "CALIBRATION",
          outcome: bOutcome,
        });
      else
  ...
  it("maps provider, workflow, skill, task, and local runtime costs consistently", () => {
    const { service } = setup();
    expect(
      service.normalizedCost({ kind: "MODEL_INFERENCE", externalCostUsd: 0.004 }),
    ).toBe(4);
    expect(service.normalizedCost({ kind: "LOCAL_INFERENCE", runtimeMs: 2_200 })).toBe(
      3,
    );
    expect(
      service.normalizedCost({ kind: "WORKFLOW_EXECUTION", runtimeMs: 1_200 }),
    ).toBe(2);
    expect(
      service.normalizedCost({ kind: "SKILL_EXECUTION", totalTokens: 1_400 }),
    ).toBe(2);
  ...
  it("attributes a provider request through reserve and settlement", async () => {
    const { service } = setup();
    await activateFunded(service);
    const reservationId = await service.reserveProviderCost({
      ownerId,
      agentId: "agent_a",
    ...
    await service.settleProviderCost({
      ownerId,
      agentId: "agent_a",
    ...
    expect(
      dashboard.ledger.some(
        (entry) => entry.references.providerRequestId === "provider-request-1",
      ),
    ).toBe(true);
  });
  ...
  it("distributes one bounded reward across contributors without inflation", async () => {
    ...
    const outcome = (agent: string) => ({
      ...
      qualityScore: 0.9,
      verificationResult: "VERIFIED" as const,
      evidenceRefs: [`review-${agent}`],
    });
    await service.distributeVerifiedReward({
      ownerId,
      totalAmount: 100,
      authority: "WORKFLOW_EVALUATOR",
      idempotencyKey: "shared:task-1",
      reasonCode: "SHARED_RESULT",
      contributions: [
        { agentId: "agent_a", weight: 3, outcome: outcome("a") },
        { agentId: "agent_b", weight: 1, outcome: outcome("b") },
      ],
    });
    const dashboard = await service.dashboard(ownerId);
    ...
    expect(
      dashboard.ledger
        .filter((entry) => entry.type === "REWARD_EARNED")
  ...
  it("runs a deterministic 30-task economy simulation without rewarding activity", async () => {
    ...
    for (const agentId of ["agent_a", "agent_b", "agent_c", "agent_d", "agent_e"])
      await activateFunded(service, agentId, 100);
    ...
    for (let index = 0; index < 30; index += 1) {
      ...
      const reserved = await service.reserve({
        ...
        costType: "TASK_EXECUTION",
        reasonCode: "SIM_TASK",
        idempotencyKey: `simulation:reserve:${index}`,
        references: { taskId: `simulation-${index}` },
      });
      await service.settle({
        ownerId,
        agentId,
        reservationId: reserved.reservation.id,
        actualCost: cost,
        idempotencyKey: `simulation:settle:${index}`,
        reasonCode: "SIM_TASK_ACTUAL",
        references: { taskId: `simulation-${index}` },
      });
      const outcome = {
        taskId: `simulation-${index}`,
        predictedSuccessProbability: 0.6 + (index % 3) * 0.1,
        estimatedCost: cost + 2,
        estimatedDurationMs: 100,
        actualSuccess: index % 5 !== 0,
        actualCost: cost,
        actualDurationMs: 80 + index,
        qualityScore: index % 5 === 0 ? 0.4 : 0.8,
        verificationResult: "VERIFIED" as const,
        evidenceRefs: [`simulation-review-${index}`],
      };
      if (outcome.actualSuccess)
        await service.rewardVerified({
          ...
          amount: 5,
          authority: "TASK_VERIFIER",
          idempotencyKey: `simulation:reward:${index}`,
          reasonCode: "SIM_VERIFIED",
          outcome,
        });
      else
        await service.penalize({
          ...
          amount: 1,
          authority: "TASK_VERIFIER",
          idempotencyKey: `simulation:penalty:${index}`,
          reasonCode: "BUSINESS_FAILURE",
          outcome,
        });
    }
    const dashboard = await service.dashboard(ownerId);
    expect(
      dashboard.performance.reduce((sum, item) => sum + item.tasksAttempted, 0),
    ).toBe(30);
    expect(dashboard.overview.reservedCredits).toBe(0);
    expect(dashboard.ledger.some((entry) => entry.reasonCode.includes("message"))).toBe(
      false,
    );
  });
});
