...
export class PostgresAgentEconomyStore implements AgentEconomyStore {
  ...
  async saveAccount(account: AgentEconomyAccount) {
    ...
    await this.pool.query(
      `INSERT INTO agent_economy_accounts(owner_id,agent_id,economy_status,available_credits,reserved_credits,lifetime_earned,lifetime_spent,reputation,created_at,updated_at,record,company_id)
       ...
       ON CONFLICT (owner_id,company_id,agent_id) DO NOTHING`,
  ...
  async updateAccount(account: AgentEconomyAccount) {
    ...
    if (!result.rows[0])
      throw this.error("ECONOMY_ACCOUNT_NOT_FOUND", "Economy account not found.");
    return AgentEconomyAccountSchema.parse(result.rows[0].record);
  }

  async findAccount(ownerId: string, agentId: string) {
    const result = await this.pool.query<RecordRow>(
      "SELECT record FROM agent_economy_accounts WHERE owner_id=$1 AND agent_id=$2 AND ($3::uuid IS NULL OR company_id=$3)",
      [ownerId, agentId, companyScope.companyId(ownerId) ?? null],
    );
    return result.rows[0]
      ? AgentEconomyAccountSchema.parse(result.rows[0].record)
      : undefined;
  }

  async listAccounts(ownerId: string) {
    const result = await this.pool.query<RecordRow>(
      "SELECT record FROM agent_economy_accounts WHERE owner_id=$1 AND ($2::uuid IS NULL OR company_id=$2) ORDER BY agent_id",
  ...
  async reserveAtomic(input: {
    ...
    try {
      ...
      const account = await this.lockAccount(
        client,
        input.account.ownerId,
        input.account.agentId,
      );
      const companyId = companyScope.companyId(input.reservation.ownerId) ?? null;
      const duplicate = await client.query<RecordRow>(
        "SELECT record FROM agent_economy_reservations WHERE owner_id=$1 AND idempotency_key=$2 AND ($3::uuid IS NULL OR company_id=$3)",
        [input.reservation.ownerId, input.reservation.idempotencyKey, companyId],
      );
      if (duplicate.rows[0]) {
        const reservation = AgentEconomyReservationSchema.parse(
          duplicate.rows[0].record,
        );
        await client.query("COMMIT");
  ...
  async listLedger(ownerId: string, limit: number) {
    const result = await this.pool.query<RecordRow>(
      "SELECT record FROM agent_economy_ledger WHERE owner_id=$1 AND ($3::uuid IS NULL OR company_id=$3) ORDER BY created_at DESC LIMIT $2",
  ...
  async listReservations(ownerId: string, agentId?: string) {
    const result = agentId
      ? await this.pool.query<RecordRow>(
          "SELECT record FROM agent_economy_reservations WHERE owner_id=$1 AND agent_id=$2 AND ($3::uuid IS NULL OR company_id=$3) ORDER BY created_at DESC",
          [ownerId, agentId, companyScope.companyId(ownerId) ?? null],
        )
      : await this.pool.query<RecordRow>(
          "SELECT record FROM agent_economy_reservations WHERE owner_id=$1 AND ($2::uuid IS NULL OR company_id=$2) ORDER BY created_at DESC",
          [ownerId, companyScope.companyId(ownerId) ?? null],
        );
  ...
  async savePerformance(record: AgentEconomyPerformance) {
    ...
    await this.pool.query(
      `INSERT INTO agent_economy_performance(owner_id,agent_id,updated_at,record,company_id) VALUES ($1,$2,$3,$4,$5)
       ON CONFLICT (owner_id,company_id,agent_id) DO UPDATE SET updated_at=$3,record=$4`,
      [
        parsed.ownerId,
        parsed.agentId,
        parsed.updatedAt,
  ...
  async findPerformance(ownerId: string, agentId: string) {
    const result = await this.pool.query<RecordRow>(
      "SELECT record FROM agent_economy_performance WHERE owner_id=$1 AND agent_id=$2 AND ($3::uuid IS NULL OR company_id=$3)",
      [ownerId, agentId, companyScope.companyId(ownerId) ?? null],
    );
    return result.rows[0]
      ? AgentEconomyPerformanceSchema.parse(result.rows[0].record)
      : undefined;
  }

  async listPerformance(ownerId: string) {
    const result = await this.pool.query<RecordRow>(
      "SELECT record FROM agent_economy_performance WHERE owner_id=$1 AND ($2::uuid IS NULL OR company_id=$2) ORDER BY agent_id",
      [ownerId, companyScope.companyId(ownerId) ?? null],
    );
    return result.rows.map((row) => AgentEconomyPerformanceSchema.parse(row.record));
  }

  private async creditOrPenalty(
    input: { account: AgentEconomyAccount; entry: AgentEconomyLedgerEntry },
    mode: "credit" | "penalty",
  ) {
    const client = await this.pool.connect();
    try {
      ...
      const existing = await this.findLedgerByKey(
        client,
        input.entry.ownerId,
        input.entry.idempotencyKey,
      );
      if (existing) {
        await client.query("COMMIT");
      ...
      const amount =
        mode === "penalty"
          ? Math.min(account.availableCredits, input.entry.amount)
          : input.entry.amount;
      if (amount <= 0)
        throw this.error(
          "INSUFFICIENT_ECONOMIC_BUDGET",
          "No available credits may be penalized.",
        );
      ...
      const updated = AgentEconomyAccountSchema.parse({
        ...
        availableCredits:
          mode === "credit"
            ...
            : account.availableCredits - amount,
        lifetimeEarned:
          mode === "credit" ? account.lifetimeEarned + amount : account.lifetimeEarned,
        lifetimeSpent:
          mode === "penalty" ? account.lifetimeSpent + amount : account.lifetimeSpent,
  ...
  private async finishReservation(
    ...
    try {
      ...
      const result = await client.query<RecordRow>(
        "SELECT record FROM agent_economy_reservations WHERE owner_id=$1 AND agent_id=$2 AND id=$3 AND ($4::uuid IS NULL OR company_id=$4) FOR UPDATE",
        [input.ownerId, input.agentId, input.reservationId, companyId],
      );
      if (!result.rows[0])
        throw this.error(
          "ECONOMY_RESERVATION_NOT_FOUND",
          "Economy reservation not found.",
        );
      ...
      if (reservation.status !== "ACTIVE")
        throw this.error(
          "ECONOMY_RESERVATION_NOT_ACTIVE",
      ...
      if (extra > account.availableCredits)
        throw this.error(
          "INSUFFICIENT_ECONOMIC_BUDGET",
          "Insufficient economic budget for settlement.",
        );
      ...
      const updated = AgentEconomyAccountSchema.parse({
        ...
        lifetimeSpent: account.lifetimeSpent + actual,
        updatedAt: input.updatedAt,
      });
      const finished = AgentEconomyReservationSchema.parse({
        ...reservation,
        amountSettled: actual,
        status,
        updatedAt: input.updatedAt,
      });
      await this.writeAccount(client, updated);
      await this.insertLedger(client, input.entry);
      await client.query(
        "UPDATE agent_economy_reservations SET status=$4,amount_settled=$5,updated_at=$6,record=$7 WHERE owner_id=$1 AND agent_id=$2 AND id=$3 AND ($8::uuid IS NULL OR company_id=$8)",
        [
          input.ownerId,
          input.agentId,
          input.reservationId,
  ...
  private async lockAccount(client: PoolClient, ownerId: string, agentId: string) {
    const result = await client.query<RecordRow>(
      "SELECT record FROM agent_economy_accounts WHERE owner_id=$1 AND agent_id=$2 AND ($3::uuid IS NULL OR company_id=$3) FOR UPDATE",
      [ownerId, agentId, companyScope.companyId(ownerId) ?? null],
    );
    if (!result.rows[0])
      throw this.error("ECONOMY_ACCOUNT_NOT_FOUND", "Economy account not found.");
  ...
  private async writeAccount(client: PoolClient, account: AgentEconomyAccount) {
    await client.query(
      ...
      [
        ...
        companyScope.companyId(account.ownerId) ?? null,
      ],
    );
  }
  ...
  private async insertLedger(client: PoolClient, entry: AgentEconomyLedgerEntry) {
    ...
    await client.query(
      "INSERT INTO agent_economy_ledger(id,owner_id,agent_id,entry_type,amount,idempotency_key,created_at,record,company_id) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)",
      [
        parsed.id,
        parsed.ownerId,
        parsed.agentId,
  ...
  private async findLedgerByKey(
    client: PoolClient,
    ownerId: string,
    idempotencyKey: string,
  ) {
    const result = await client.query<RecordRow>(
      "SELECT record FROM agent_economy_ledger WHERE owner_id=$1 AND idempotency_key=$2 AND ($3::uuid IS NULL OR company_id=$3)",
      [ownerId, idempotencyKey, companyScope.companyId(ownerId) ?? null],
    );
    return result.rows[0]
      ? AgentEconomyLedgerEntrySchema.parse(result.rows[0].record)
      : undefined;
  }
